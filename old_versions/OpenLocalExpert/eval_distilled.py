#!/usr/bin/env python3
# eval_distilled.py - 蒸馏LoRA vs 传统RAG 评估
...
def relevance_score(retrieved, related_terms, model):
    scores = [semantic_similarity(retrieved, term, model) for term in related_terms]
...
for i, case in enumerate(TEST_CASES):
    ...
    if chunk_id is not None and routed_content:
        routed_text = routed_content
    else:
        routed_text = baseline_text
    ...
    print(f"[{i+1:2d}] {q}")
    print(f"    Chunk: {f'Chunk_{chunk_id:04d}' if chunk_id else 'N/A'} | "
          f"基线: {baseline_score:.3f} | 路由: {routed_score:.3f} ({symbol}{abs(diff):.3f})")
    print()
...
print(f"  蒸馏LoRA: {avg_routed:.4f} ({'+' if avg_routed > avg_baseline else ''}{avg_routed - avg_baseline:.4f})")
...
if avg_routed > avg_baseline + 0.02:
    print(f"✅ 蒸馏LoRA优于传统RAG")
elif avg_routed < avg_baseline - 0.02:
    print(f"⚠️ 传统RAG优于蒸馏LoRA")
else:
    print(f"➡️ 两者效果接近")
