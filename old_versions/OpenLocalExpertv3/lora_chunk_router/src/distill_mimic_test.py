"""
Distill Mimic Test - 模仿测试集风格生成问题
解决训练问题与测试问题分布不匹配的问题
"""
...
def build_9_questions_mimic_test(chunk_id: str, title: str, text: str) -> List[Dict]:
    """为chunk构建9个模仿测试集风格的问题"""
    # 从title提取话题
    ...
    # 3 factual - 测试集风格
    questions.append(f"What did he say about {topics[0]}?")
    questions.append(f"What about {topics[1] if len(topics) > 1 else topics[0]}?")
    questions.append(f"What's {topics[2] if len(topics) > 2 else topics[0]}?")

    # 3 fuzzy - 测试集风格
    questions.append(f"Can you explain {topics[0]}?")
    questions.append(f"What was the deal with {topics[1] if len(topics) > 1 else topics[0]}?")
    questions.append(f"Tell me about {topics[2] if len(topics) > 2 else topics[0]}")

    # 3 intent - 测试集风格
    questions.append(f"Why {topics[0]}?")
    questions.append(f"Why does it matter?")
    questions.append(f"What's the point?")
...
def distill_dataset(chunks: List[Dict], output_path: str, verbose: bool = True) -> List[Dict]:
    ...
    for i, chunk in enumerate(chunks):
        ...
        # 生成问题
        samples = build_9_questions_mimic_test(chunk_id, title, text)
...
def main():
    ...
    print("=" * 60)
    print("Distill Mimic Test - 模仿测试集风格问题")
    print("=" * 60)
    ...
    for i in range(min(5, len(samples))):
        print(f"  - {samples[i]['instruction']} -> {samples[i]['output']}")
